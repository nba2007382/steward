...
class user {
    async activeEmail(req, res, next) {
        try {
            ...
            if (!code || !account) {
                res.render('active', { message: '链接异常' })
                return
            }
            verifyActiveToken(res, code, account, userModel)
            return
        } catch (err) {
            res.status(400).send({
                message: '激活出错' + err
        ...
        try {
            ...
            if (user.length !== 0) {
                if (user[0].status == 0) {
                    ...
                    const emailContent = `<div><a>${localhost}/user/register/active?code='${addActiveToken(user_email)}"&account="${user_email}"</a></div>`
            ...
            try {
                ...
                const emailContent = `<div><a>${localhost}/user/register/active?code=${code}&account=${ useremail}</a></div>`
                console.log('11111');
